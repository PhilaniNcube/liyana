...
export async function GET(
  ...
  drawLinePage1('Registration Number', '2023/790223/07', 70);
  drawLinePage1('NCR Registration Number', 'NCRCP18217', 70);
  drawLinePage1('Physical Address', '1 Hunter Street | Nelspruit | 1200 | Mpumalanga | South Africa', 70);
  drawLinePage1('Contact Details', '012 004 0889', 70);
  addSpacePage1(10);
  ...
  (async () => {
    try {
      ...
      const storagePath = `profile-documents/${profileId}/contract-loan-credit-agreement-${loanId}-${timestamp}.pdf`;
      ...
      if (uploadError) {
        console.error('[credit-agreement][upload] Failed to upload PDF:', uploadError);
        return;
      }
      ...
      if (insertError) {
        console.error('[credit-agreement][insert] Failed to insert profile_documents record:', insertError);
      }
    } catch (e) {
      console.error('[credit-agreement] Unexpected error during upload/record:', e);
    }
  })();
